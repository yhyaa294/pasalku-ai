...
export const HeroSection: FC<HeroSectionProps> = ({ onGetStarted }) => {
  ...
  // Rotating titles - berbagai variasi pesan
  const rotatingTitles = [
    {
      line1: "Jangan Biarkan",
      highlight: "Kebingungan Hukum",
      line2: "Merugikan Anda"
    },
    {
      line1: "Revolusi Hukum",
      highlight: "Digital Indonesia",
      line2: "Dimulai dari Sini"
    },
    {
      line1: "Akses Keadilan",
      highlight: "Mudah & Terjangkau",
      line2: "untuk Semua"
    },
    {
      line1: "Waktunya",
      highlight: "Solusi Hukum Cerdas",
      line2: "dengan Teknologi AI"
    }
  ]

  useEffect(() => {
    // CRITICAL: Set mounted first to prevent hydration mismatch
    setIsMounted(true)
    
    // Delay all browser-specific logic to ensure hydration is complete
    initTimeoutRef.current = setTimeout(() => {
      let supportsFinePointer = false
      
      // Check pointer support only after component mounts - PROTECTED
      ...
      const handleMouseMove = (e: MouseEvent) => {
        if (!supportsFinePointer || !isMounted) return
        ...
        if (rafRef.current == null) {
          rafRef.current = requestAnimationFrame(() => {
            ...
            if (evt && sectionRef.current) {
              ...
              // Update CSS variables for transforms - PROTECTED
              try {
      ...
      // Auto-rotate titles setiap 4 detik
  ...
  return (
  <section ref={sectionRef} id="hero" className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-b from-white via-blue-50/30 to-purple-50/20 dark:from-gray-900 dark:via-blue-950/30 dark:to-purple-950/20 pt-20 transition-colors duration-500">
      ...
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_80%_20%,rgba(139,92,246,0.1),transparent_50%)] dark:bg-[radial-gradient(circle_at_80%_20%,rgba(139,92,246,0.2),transparent_50%)]"></div>
  ...
  <div className="absolute inset-0 bg-[linear-gradient(rgba(59,130,246,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(59,130,246,0.03)_1px,transparent_1px)] bg-[size:64px_64px] [mask-image:radial-gradient(ellipse_80%_50%_at_50%_50%,black,transparent)]"></div>
      ...
      {/* Floating Orbs with Mouse Parallax - hidden on mobile / reduced motion */}
      <div 
        className="absolute top-1/4 left-1/4 w-96 h-96 bg-blue-500/10 rounded-full blur-3xl motion-safe:animate-pulse hidden md:block motion-reduce:hidden transform translate-x-[var(--orb1-x)] translate-y-[var(--orb1-y)]"
      ></div>
      <div 
        className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-purple-500/10 rounded-full blur-3xl motion-safe:animate-pulse hidden md:block motion-reduce:hidden motion-safe:[animation-delay:1s] transform translate-x-[var(--orb2-x)] translate-y-[var(--orb2-y)]"
      ></div>

      <div className="relative z-10 max-w-7xl mx-auto px-4 py-20">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          {/* Left Content - Psychology Enhanced */}
          <div className="text-center lg:text-left space-y-8 animate-safe-fade-in">
            {/* Badge - Prototype/Launch Year */}
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/80 backdrop-blur-sm border border-blue-200/50 shadow-lg">
              ...
              <span className="text-sm font-semibold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                ✨ Rilis 2025 • Platform Prototype
              </span>
              <Sparkles className="w-4 h-4 text-blue-500" />
            </div>

            {/* Hero Title - ROTATING TITLES with Smooth Animation */}
            <h1 className="text-5xl md:text-6xl lg:text-7xl font-black leading-[1.1] tracking-tight min-h-[280px] md:min-h-[320px]">
              <span 
                key={`line1-${currentTitleIndex}`}
                className="block text-gray-900 dark:text-gray-100 mb-2 motion-safe:animate-title-fade-in"
              >
                {rotatingTitles[currentTitleIndex].line1}
              </span>
              <span 
                key={`highlight-${currentTitleIndex}`}
                className="block bg-gradient-to-r from-red-600 via-orange-600 to-red-600 dark:from-orange-400 dark:via-red-400 dark:to-orange-400 bg-clip-text text-transparent animate-safe-gradient-x bg-[length:200%_auto0%_auto] motion-safe:animate-title-fade-in motion-safe:[animation-delay:0.1s]"
              >
                {rotatingTitles[currentTitleIndex].highlight}
              </span>
              <span 
                key={`line2-${currentTitleIndex}`}
                className="block text-gray-900 dark:text-gray-100 mt-2 motion-safe:animate-title-fade-in motion-safe:[animation-delay:0.2s]"
              >
            ...
            <p className="text-xl md:text-2xl text-gray-600 dark:text-gray-300 max-w-2xl leading-relaxed font-medium">
              Dapatkan <span className="text-blue-600 dark:text-blue-400 font-semibold">jawaban hukum yang jelas</span> dalam hitungan detik. Platform AI yang membantu Anda <span className="text-purple-600 dark:text-purple-400 font-semibold">memahami hak-hak</span> dan <span className="text-blue-600 dark:text-blue-400 font-semibold">melindungi kepentingan</span> Anda tanpa biaya mahal.
            </p>

            {/* CTA Buttons - RISK REDUCTION */}
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <div className="flex flex-col gap-2">
                <button
                  onClick={onGetStarted}
                  className="group relative px-8 py-4 bg-gradient-to-r from-blue-600 via-purple-600 to-blue-600 bg-[length:200%_auto] text-white rounded-2xl font-bold text-lg hover:shadow-2xl hover:shadow-blue-500/50 transition-all duration-300 transform hover:scale-105 motion-safe:animate-gradient-x overflow-hidden"
                >
              ...
              <button onClick={onGetStarted} className="group px-8 py-4 bg-white/80 backdrop-blur-sm text-gray-700 rounded-2xl font-semibold text-lg border-2 border-gray-200 hover:border-blue-300 hover:bg-white hover:text-blue-600 transition-all duration-300 hover:shadow-lg">
          ...
          <div className="relative animate-safe-fade-in-delayed">
            ...
            <div className="relative">
              ...
              <div className="relative grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2 relative group bg-white/90 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl p-8 border border-gray-200/50 dark:border-gray-700 shadow-2xl hover:shadow-blue-500/30 transition-all duration-500 hover:-translate-y-2 overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="flex items-center gap-4 mb-6">
                      <div className="relative w-20 h-20 bg-gradient-to-br from-blue-500 via-purple-500 to-blue-600 rounded-2xl flex items-center justify-center shadow-2xl group-hover:scale-110 group-hover:rotate-3 transition-all duration-500">
                        <span className="text-4xl motion-safe:animate-pulse">🤖</span>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      {/* Stat 1 - 24/7 */}
                      <div className="group/stat relative text-center p-4 bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-2xl border border-blue-200/50 dark:border-blue-800/50 hover:border-blue-300 hover:shadow-lg hover:shadow-blue-200/50 transition-all duration-300 hover:-translate-y-1 cursor-pointer overflow-hidden">
                      ...
                      <div className="group/stat relative text-center p-4 bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 rounded-2xl border border-purple-200/50 dark:border-purple-800/50 hover:border-purple-300 hover:shadow-lg hover:shadow-purple-200/50 transition-all duration-300 hover:-translate-y-1 cursor-pointer overflow-hidden">
                      ...
                      {/* Stat 3 - Accuracy */}
                      <div className="group/stat relative text-center p-4 bg-gradient-to-br from-green-50 to-emerald-100 dark:from-emerald-900/20 dark:to-emerald-800/20 rounded-2xl border border-green-200/50 dark:border-emerald-800/50 hover:border-green-300 hover:shadow-lg hover:shadow-green-200/50 transition-all duration-300 hover:-translate-y-1 cursor-pointer overflow-hidden">
                ...
                <div className="relative group bg-white/90 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl p-6 border border-amber-200/50 dark:border-amber-800/40 shadow-xl hover:shadow-amber-500/30 transition-all duration-500 hover:-translate-y-2 overflow-hidden cursor-pointer">
                  {/* Animated background */}
                  ...
                  <div className="relative z-10">
                    ...
                    <h4 className="font-black text-lg text-gray-900 dark:text-gray-100 mb-2 group-hover:text-amber-700 transition-colors">Auto Citations</h4>
                    <p className="text-sm text-gray-600 dark:text-gray-300 font-medium">Referensi hukum otomatis</p>
                    <div className="mt-3 flex items-center gap-1 text-xs text-amber-600 font-semibold">
                      <span className="inline-block w-1.5 h-1.5 bg-amber-500 rounded-full motion-safe:animate-pulse"></span>
                      Akurat & Terpercaya
                    </div>
                  </div>
                </div>
                ...
                <div className="relative group bg-white/90 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl p-6 border border-green-200/50 dark:border-green-800/40 shadow-xl hover:shadow-green-500/30 transition-all duration-500 hover:-translate-y-2 overflow-hidden cursor-pointer">
                  {/* Animated background */}
                  ...
                  <div className="relative z-10">
                    ...
                    <h4 className="font-black text-lg text-gray-900 dark:text-gray-100 mb-2 group-hover:text-green-700 transition-colors">AI Predictions</h4>
                    <p className="text-sm text-gray-600 dark:text-gray-300 font-medium">Prediksi hasil kasus</p>
                    <div className="mt-3 flex items-center gap-1 text-xs text-green-600 font-semibold">
                      <span className="inline-block w-1.5 h-1.5 bg-green-500 rounded-full motion-safe:animate-pulse"></span>
        ...
        <div className="mt-20 text-center animate-safe-fade-in anim-delay-0-5s">
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 font-medium">Dipercaya oleh pengguna di seluruh Indonesia</p>
          <div className="flex justify-center items-center gap-12 flex-wrap">
            <div className="text-center group cursor-pointer">
              ...
              <div className="text-sm text-gray-600 dark:text-gray-300 font-medium">Fitur AI</div>
            </div>
            ...
            <div className="text-center group cursor-pointer">
              ...
              <div className="text-sm text-gray-600 dark:text-gray-300 font-medium">Support</div>
            </div>
            <div className="w-px h-12 bg-gradient-to-b from-transparent via-gray-300 dark:via-gray-700 to-transparent"></div>
            <div className="text-center group cursor-pointer">
              ...
              <div className="text-sm text-gray-600 dark:text-gray-300 font-medium">Akurasi</div>
            </div>
          </div>
        </div>
      </div>
